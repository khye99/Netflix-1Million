...
train = sc.textFile(sys.argv[1]).sample(False,0.01).map(lambda line: line.split(',')).persist()
test = sc.textFile(sys.argv[2]).sample(False,0.01).map(lambda line: line.split(',')).persist()
